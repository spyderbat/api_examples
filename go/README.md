...
```golang
import sbapi "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
```golang
ctx := context.WithValue(context.Background(), sbapi.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), sbapi.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```
ctx := context.WithValue(context.Background(), sbapi.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), sbapi.ContextOperationServerVariables, map[string]map[string]string{
...
*InternalAPIApi* | [**LogData**](docs/InternalAPIApi.md#logdata) | **Post** /api/v1/_/log | Logs data
...
*UserApi* | [**UserAuth**](docs/UserApi.md#userauth) | **Post** /api/v1/app/user/auth | Authenticate a user, returns token required for authentication
...
*UserApi* | [**UserSignup**](docs/UserApi.md#usersignup) | **Get** /api/v1/app/signup | Sign up a new user
...
## Documentation For Models
 ...
 - [User](docs/User.md)
 - [UserAuthInput](docs/UserAuthInput.md)
